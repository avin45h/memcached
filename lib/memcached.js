...
Client.config = {
  ...
  , encoding: 'utf8'           // use encoding utf8 unless specified
  , zlibInflate: false           // use zlib inflate only if specified, defaults to false
    ...
    while(S.bufferArray.length && privates.allCommands.test(S.bufferArray[0])) {
      ...
      if (privates.parsers[tokenSet[0]]) {
        ...
        if (tokenSet[0] === 'VALUE') {
          value  = "";
          var currentVal = null;
          var multiLineData = false;
          while(S.bufferArray[0] !== 'END' && S.bufferArray[0].indexOf("VALUE") !== 0){
            currentVal = S.bufferArray.shift();
            value += multiLineData ? LINEBREAK + currentVal : currentVal;
            multiLineData = true;
          }


          if(value.length){
            S.bufferArray.unshift(value);
          }
          value = S.bufferArray.shift();
          if(this.zlibInflate){
            try{
              value = zlib.inflateSync(new Buffer(value, "binary")).toString() || value;
            }catch(e){
            }
          }
